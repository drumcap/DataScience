...
class CommentDB(object):
    ...
    def save_comment_info(self, productno, comment_writer, comment_grade):
        if self.get_comment_info(productno, comment_writer):
            session = Session()
            insert_comment = Comment(ProductNo = productno, Writer = comment_writer, Grade = comment_grade, Enrolltime = crawltime)
    ...
    def update_comment_info(self, productno, comment_writer, comment_grade):
        session = Session()
        update_comment = session.query(Comment).filter(Comment.ProductNo == productno, Comment.Writer == comment_writer).one()
    ...
    def get_comment_info(self, productno, comment_writer):
        session = Session()
        find_comment = session.query(Comment).filter(Comment.ProductNo == productno, Comment.Writer == comment_writer).all()
