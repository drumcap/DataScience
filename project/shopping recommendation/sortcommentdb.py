...
class SortCommentDB(object):
    ...
    def sort_count(self):
        ...
        for product in product_list:
            find_count = session.query(Comment).filter(Comment.ProductNo == product.ProductNo).count()
            print find_count
            update_product = session.query(Product).filter(Product.ProductNo == product.ProductNo).one()
